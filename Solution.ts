...
function majorityFrequencyGroup(input: string): string {
    const ALPHABET_SIZE = 26;
    const ASCII_LOWER_CASE_A = 97;
    const frequencyLetters: number[] = new Array(ALPHABET_SIZE).fill(0);
    for (let i = 0; i < input.length; ++i) {
        ++frequencyLetters[input.codePointAt(i) - ASCII_LOWER_CASE_A];
    }

    const frequencyLettersToGroupSize: number[] = new Array(input.length + 1).fill(0);
    ...
    const uniqueLetters: string[] = new Array();
